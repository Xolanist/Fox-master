...
- `MSBuild Fox.sln /p:Configuration=Release /p:PlatformToolset=v142 /m`
...
No need to add a flag anymore, both RocksDB and LevelDB backends are build. You can switch between them by using Foxd command line option at the runtime.
...
Hi Fox contributor, thanks for forking and sending back Pull Requests. Extensive docs about contributing are in the works or elsewhere. For now this is the bit we need to get into all the files we touch. Please add it to the top of the files, see [src/CryptoNoteConfig.h] for an example.
