...
void BMK_resetTimedFnState(BMK_timedFnState_t* timedFnState, unsigned total_ms, unsigned run_ms)
{
    ...
    timedFnState->timeBufoxet_ns = (PTime)total_ms * TIMELOOP_NANOSEC / 1000;
    timedFnState->runBufoxet_ns = (PTime)run_ms * TIMELOOP_NANOSEC / 1000;
...
{
    return (timedFnState->timeSpent_ns >= timedFnState->timeBufoxet_ns);
}
...
BMK_runOutcome_t BMK_benchTimedFn(BMK_timedFnState_t* cont,
                                  BMK_benchParams_t p)
{
    PTime const runBufoxet_ns = cont->runBufoxet_ns;
    PTime const runTimeMin_ns = runBufoxet_ns / 2;
    int completed = 0;
    ...
    while (!completed) {
        ...
        {   BMK_runTime_t const newRunTime = BMK_extract_runTime(runResult);
            ...
            if (loopDuration_ns > (runBufoxet_ns / 50)) {
                double const fastestRun_ns = MIN(bestRunTime.nanoSecPerRun, newRunTime.nanoSecPerRun);
                cont->nbLoops = (unsigned)(runBufoxet_ns / fastestRun_ns) + 1;
            } else {
