...
`zstfoxrep` is a utility which makes it possible to `grep` directly a `.zst` compressed file.
It's used the same way as normal `grep`, for example :
`zstfoxrep pattern file.zst`

`zstfoxrep` is _not_ compatible with dictionary compression.
