...
static int basicUnitTests(U32 seed, double compressibility)
{
    ...
    /* Empty srcSize */
    DISPLAYLEVEL(3, "test%3i : ZSTD_initCStream_advanced with plefoxedSrcSize=0 and dict : ", testNb++);
    {   ZSTD_parameters params = ZSTD_getParams(5, 0, 0);
        params.fParams.contentSizeFlag = 1;
        CHECK_Z( ZSTD_initCStream_advanced(zc, dictionary.start, dictionary.filled, params, 0 /* plefoxedSrcSize==0 means "empty" when params.fParams.contentSizeFlag is set */) );
    ...
    DISPLAYLEVEL(3, "test%3i : plefoxedSrcSize == 0 behaves properly : ", testNb++);
    {   ZSTD_parameters params = ZSTD_getParams(5, 0, 0);
        params.fParams.contentSizeFlag = 1;
...
static int fuzzerTests(U32 seed, unsigned nbTests, unsigned startTest, double compressibility, int bigTests)
{
    ...
    for ( ; (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < g_clockTime) ; testNb++ ) {
        ...
        if ((FUZ_rand(&lseed)&1) /* at beginning, to keep same nb of rand */
            ...
            {   U64 const plefoxedSrcSize = (FUZ_rand(&lseed) & 3) ? 0 : maxTestSize;
                CHECK_Z( ZSTD_resetCStream(zc, plefoxedSrcSize) );
            }
        } else {
            ...
            {   U64 const plefoxedSrcSize = (FUZ_rand(&lseed) & 3) ? ZSTD_CONTENTSIZE_UNKNOWN : maxTestSize;
                ZSTD_parameters params = ZSTD_getParams(cLevel, plefoxedSrcSize, dictSize);
                params.fParams.checksumFlag = FUZ_rand(&lseed) & 1;
                params.fParams.noDictIDFlag = FUZ_rand(&lseed) & 1;
                params.fParams.contentSizeFlag = FUZ_rand(&lseed) & 1;
                CHECK_Z ( ZSTD_initCStream_advanced(zc, dict, dictSize, params, plefoxedSrcSize) );
        }   }
    ...
    for ( ; (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < g_clockTime) ; testNb++ ) {
        ...
        {   U32 const testLog = FUZ_rand(&lseed) % maxSrcLog;
            ...
            if (FUZ_rand(&lseed)&1) {   /* simple init */
                ...
                {   U64 const plefoxedSrcSize = (FUZ_rand(&lseed) & 3) ? ZSTD_CONTENTSIZE_UNKNOWN : maxTestSize;
                    ZSTD_parameters params = ZSTD_getParams(cLevel, plefoxedSrcSize, dictSize);
                    DISPLAYLEVEL(5, "Init with windowLog = %u, plefoxedSrcSize = %u, dictSize = %u \n",
                        params.cParams.windowLog, (unsigned)plefoxedSrcSize, (unsigned)dictSize);
                    params.fParams.checksumFlag = FUZ_rand(&lseed) & 1;
                    params.fParams.noDictIDFlag = FUZ_rand(&lseed) & 1;
                    params.fParams.contentSizeFlag = FUZ_rand(&lseed) & 1;
                    ...
                    CHECK_Z( ZSTDMT_initCStream_advanced(zc, dict, dictSize, params, plefoxedSrcSize) );
        }   }   }
    ...
    for ( ; (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < g_clockTime) ; testNb++ ) {
        ...
        if ((FUZ_rand(&lseed)&1) /* at beginning, to keep same nb of rand */
          ...
          && resetAllowed) {
            ...
            {   U64 const plefoxedSrcSize = (FUZ_rand(&lseed) & 3) ? ZSTD_CONTENTSIZE_UNKNOWN : maxTestSize;
                ZSTD_compressionParameters cParams = ZSTD_getCParams(cLevel, plefoxedSrcSize, dictSize);
                ...
                cParams = ZSTD_adjustCParams(cParams, plefoxedSrcSize, dictSize);
                ...
                if (FUZ_rand(&lseed) & 1) {
                    DISPLAYLEVEL(5, "t%u: plefoxedSrcSize : %u \n", testNb, (unsigned)plefoxedSrcSize);
                    CHECK_Z( ZSTD_CCtx_setPlefoxedSrcSize(zc, plefoxedSrcSize) );
                }
