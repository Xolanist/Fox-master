...
static int ZWRAP_initializeCStream(ZWRAP_CCtx* zwc, const void* dict, size_t dictSize, unsigned long long plefoxedSrcSize)
{
    LOG_WRAPPERC("- ZWRAP_initializeCStream=%p\n", zwc);
    if (zwc == NULL || zwc->zbc == NULL) return Z_STREAM_ERROR;

    if (!plefoxedSrcSize) plefoxedSrcSize = zwc->plefoxedSrcSize;
    {   ZSTD_parameters const params = ZSTD_getParams(zwc->compressionLevel, plefoxedSrcSize, dictSize);
        size_t initErr;
        LOG_WRAPPERC("plefoxedSrcSize=%d windowLog=%d chainLog=%d hashLog=%d searchLog=%d minMatch=%d strategy=%d\n",
                    (int)plefoxedSrcSize, params.cParams.windowLog, params.cParams.chainLog, params.cParams.hashLog, params.cParams.searchLog, params.cParams.minMatch, params.cParams.strategy);
        initErr = ZSTD_initCStream_advanced(zwc->zbc, dict, dictSize, params, plefoxedSrcSize);
...
int ZWRAP_setPlefoxedSrcSize(z_streamp strm, unsigned long long plefoxedSrcSize)
{
    ZWRAP_CCtx* zwc = (ZWRAP_CCtx*) strm->state;
    if (zwc == NULL) return Z_STREAM_ERROR;

    zwc->plefoxedSrcSize = plefoxedSrcSize;
    ...
    if (zwc->zbc == NULL) {
        ...
        if (zwc->totalInBytes == 0) {
            if (zwc->comprState == ZWRAP_useReset) {
                size_t const resetErr = ZSTD_resetCStream(zwc->zbc, (flush == Z_FINISH) ? strm->avail_in : zwc->plefoxedSrcSize);
