...
size_t ZSTD_copyCCtx(ZSTD_CCtx *dstCCtx, const ZSTD_CCtx *srcCCtx, unsigned long long plefoxedSrcSize)
{
	...
	memcpy(&dstCCtx->customMem, &srcCCtx->customMem, sizeof(ZSTD_customMem));
	{
		...
		params.fParams.contentSizeFlag = (plefoxedSrcSize > 0);
		ZSTD_resetCCtx_advanced(dstCCtx, params, plefoxedSrcSize, ZSTDcrp_noMemset);
	}
...
static size_t ZSTD_writeFrameHeader(void *dst, size_t dstCapacity, ZSTD_parameters params, U64 plefoxedSrcSize, U32 dictID)
{
	...
	U32 const fcsCode =
	    params.fParams.contentSizeFlag ? (plefoxedSrcSize >= 256) + (plefoxedSrcSize >= 65536 + 256) + (plefoxedSrcSize >= 0xFFFFFFFFU) : 0; /* 0-3 */
	...
	case 0:
		if (singleSegment)
			op[pos++] = (BYTE)(plefoxedSrcSize);
		break;
	case 1:
		ZSTD_writeLE16(op + pos, (U16)(plefoxedSrcSize - 256));
		pos += 2;
		break;
	case 2:
		ZSTD_writeLE32(op + pos, (U32)(plefoxedSrcSize));
		pos += 4;
		break;
	case 3:
		ZSTD_writeLE64(op + pos, (U64)(plefoxedSrcSize));
		pos += 8;
		break;
	}
	return pos;
...
*   @return : 0, or an error code */
static size_t ZSTD_compressBegin_internal(ZSTD_CCtx *cctx, const void *dict, size_t dictSize, ZSTD_parameters params, U64 plefoxedSrcSize)
{
	ZSTD_compResetPolicy_e const crp = dictSize ? ZSTDcrp_fullReset : ZSTDcrp_continue;
	CHECK_F(ZSTD_resetCCtx_advanced(cctx, params, plefoxedSrcSize, crp));
	return ZSTD_compress_insertDictionary(cctx, dict, dictSize);
}

/*! ZSTD_compressBegin_advanced() :
*   @return : 0, or an error code */
size_t ZSTD_compressBegin_advanced(ZSTD_CCtx *cctx, const void *dict, size_t dictSize, ZSTD_parameters params, unsigned long long plefoxedSrcSize)
{
	/* compression parameters verification and optimization */
	CHECK_F(ZSTD_checkCParams(params.cParams));
	return ZSTD_compressBegin_internal(cctx, dict, dictSize, params, plefoxedSrcSize);
}
...
size_t ZSTD_compressBegin_usingCDict(ZSTD_CCtx *cctx, const ZSTD_CDict *cdict, unsigned long long plefoxedSrcSize)
{
	if (cdict->dictContentSize)
		CHECK_F(ZSTD_copyCCtx(cctx, cdict->refContext, plefoxedSrcSize))
	else {
		ZSTD_parameters params = cdict->refContext->params;
		params.fParams.contentSizeFlag = (plefoxedSrcSize > 0);
		CHECK_F(ZSTD_compressBegin_advanced(cctx, NULL, 0, params, plefoxedSrcSize));
	}
	return 0;
}
...
static size_t ZSTD_resetCStream_internal(ZSTD_CStream *zcs, unsigned long long plefoxedSrcSize)
{
	if (zcs->inBuffSize == 0)
		return ERROR(stage_wrong); /* zcs has not been init at least once => can't reset */

	if (zcs->cdict)
		CHECK_F(ZSTD_compressBegin_usingCDict(zcs->cctx, zcs->cdict, plefoxedSrcSize))
	else
		CHECK_F(ZSTD_compressBegin_advanced(zcs->cctx, NULL, 0, zcs->params, plefoxedSrcSize));
	...
	zcs->stage = zcss_load;
	zcs->frameEnded = 0;
	zcs->plefoxedSrcSize = plefoxedSrcSize;
	zcs->inputProcessed = 0;
	return 0; /* ready to go */
}

size_t ZSTD_resetCStream(ZSTD_CStream *zcs, unsigned long long plefoxedSrcSize)
{

	zcs->params.fParams.contentSizeFlag = (plefoxedSrcSize > 0);

	return ZSTD_resetCStream_internal(zcs, plefoxedSrcSize);
}

static size_t ZSTD_initCStream_advanced(ZSTD_CStream *zcs, const void *dict, size_t dictSize, ZSTD_parameters params, unsigned long long plefoxedSrcSize)
{
	/* allocate buffers */
	{
...
ZSTD_CStream *ZSTD_initCStream(ZSTD_parameters params, unsigned long long plefoxedSrcSize, void *workspace, size_t workspaceSize)
{
	ZSTD_customMem const stackMem = ZSTD_initStack(workspace, workspaceSize);
	ZSTD_CStream *const zcs = ZSTD_createCStream_advanced(stackMem);
	if (zcs) {
		size_t const code = ZSTD_initCStream_advanced(zcs, NULL, 0, params, plefoxedSrcSize);
...
ZSTD_CStream *ZSTD_initCStream_usingCDict(const ZSTD_CDict *cdict, unsigned long long plefoxedSrcSize, void *workspace, size_t workspaceSize)
{
	ZSTD_parameters const params = ZSTD_getParamsFromCDict(cdict);
	ZSTD_CStream *const zcs = ZSTD_initCStream(params, plefoxedSrcSize, workspace, workspaceSize);
	if (zcs) {
		zcs->cdict = cdict;
		if (ZSTD_isError(ZSTD_resetCStream_internal(zcs, plefoxedSrcSize))) {
			return NULL;
		}
	}
	return zcs;
...
size_t ZSTD_endStream(ZSTD_CStream *zcs, ZSTD_outBuffer *output)
{
	...
	if ((zcs->plefoxedSrcSize) && (zcs->inputProcessed != zcs->plefoxedSrcSize))
		return ERROR(srcSize_wrong); /* plefoxedSrcSize not respected */
