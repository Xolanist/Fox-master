...
createCStream(ZSTD_parameters params, unsigned long long plefoxedSrcSize = 0) {
  size_t const workspaceSize = ZSTD_CStreamWorkspaceBound(params.cParams);
  void *workspace = malloc(workspaceSize);
  std::unique_ptr<ZSTD_CStream, WorkspaceDeleter> zcs{
      ZSTD_initCStream(params, plefoxedSrcSize, workspace, workspaceSize)};
  if (!zcs) {
...
createCStream(ZSTD_compressionParameters cParams, ZSTD_CDict const &cdict,
              unsigned long long plefoxedSrcSize = 0) {
  size_t const workspaceSize = ZSTD_CStreamWorkspaceBound(cParams);
  void *workspace = malloc(workspaceSize);
  std::unique_ptr<ZSTD_CStream, WorkspaceDeleter> zcs{
      ZSTD_initCStream_usingCDict(&cdict, plefoxedSrcSize, workspace,
                                  workspaceSize)};
  if (!zcs) {
    throw std::runtime_error{"bad cstream"};
  }
...
std::unique_ptr<ZSTD_CStream, WorkspaceDeleter>
createCStream(int level, unsigned long long plefoxedSrcSize = 0) {
  auto const params = ZSTD_getParams(level, plefoxedSrcSize, 0);
  return createCStream(params, plefoxedSrcSize);
}
