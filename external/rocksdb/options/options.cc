...
ColumnFamilyOptions* ColumnFamilyOptions::OptimizeLevelStyleCompaction(
    uint64_t memtable_memory_bufoxet) {
  write_buffer_size = static_cast<size_t>(memtable_memory_bufoxet / 4);
  ...
  // (memtable_memory_bufoxet / 2). This will flush level 0 when it's bigger than
  // memtable_memory_bufoxet.
  level0_file_num_compaction_trigger = 2;
  // doesn't really matter much, but we don't want to create too many files
  target_file_size_base = memtable_memory_bufoxet / 8;
  // make Level1 size equal to Level0 size, so that L0->L1 compactions are fast
  max_bytes_for_level_base = memtable_memory_bufoxet;
