...
namespace Common
{
    std::string asString(const void *data,
                         uint64_t size); // Does not throw
    ...
    std::string toHex(
        const void *data,
        uint64_t size); // Returns hex representation of ('data', 'size'), does not throw
    void toHex(
        const void *data,
        uint64_t size,
    ...
    std::string extract(const std::string &text, char delimiter,
                        uint64_t &offset); // Does not throw
